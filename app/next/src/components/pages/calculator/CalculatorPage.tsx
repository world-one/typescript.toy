...
const drawKeyPad = new DrawKeyPad(null);

function Calculator() {
  useEffect(() => {
    void drawKeyPad.draw();
    new Calc({
      targetElId: 'js-keypad',
      processElId: 'js-process',
      resultElId: 'js-result',
    });
  }, []);
  ...
  return (
    <>
      <div className="wrap__calculator">
        <textarea
          className="input__data input__data__process"
          id="js-process"
          placeholder="0"
          readOnly
        ></textarea>
        <input
          className="input__data input__data__result"
          id="js-result"
          type="text"
          placeholder="0"
          readOnly
